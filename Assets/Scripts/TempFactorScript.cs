...
public class TempFactorScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        #if UNITY_EDITOR
        // ✅ Ensure a fresh reset when entering Play Mode
        ...
        // ✅ If this is a brand-new game session, reset everything
        if (isNewSession)
        {
        ...
        // ✅ Ensure Root Factor is always visible
        RootFactorBlock.SetActive(true);
        factorCompletion[RootFactorBlock] = true;

        if (isNewSession)
        {
            // ✅ Ensure only the first three blocks are visible on a new session
            EnableFactorBlock(0);
            EnableFactorBlock(5);
        }
        else
        {
            // ✅ Restore previously unlocked factors from PlayerPrefs on scene reload
    ...
    private void EnableFactorBlock(int index)
    {
        ...
        // ✅ Ensure factor isn't already unlocked
